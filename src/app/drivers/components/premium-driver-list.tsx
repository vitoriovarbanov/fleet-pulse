...
import { DriversLoadingSkeleton, DriversGridSkeleton } from './drivers-loading-skeleton';
import { PremiumCreateDriverDialog } from './forms/premium/premium-create-driver-dialog';
import { PremiumEditDriverDialog } from './forms/premium/premium-edit-driver-dialog';

import type { DriverCard as DriverCardType } from '@/server/api/routers/drivers/repository/drivers.repository.types';

type DriverStatus = 'AVAILABLE' | 'ON_DUTY' | 'OFF_DUTY' | 'ON_REST' | 'SICK_LEAVE' | 'VACATION';
...
export function PremiumDriverList() {
    ...
    const { data, isLoading, isError, error, refetch, isFetching } = api.drivers.list.useQuery(
    ...
    const stats = useMemo(() => {
        ...
        return {
            ...
            onDuty: drivers.filter((d) => d.driverProfile?.driverStatus === 'ON_DUTY').length,
            withVehicle: drivers.filter((d) => d.driverProfile?.assignedVehicleId).length,
        };
    }, [data?.drivers]);
    ...
    return (
        <div className="space-y-6 sm:space-y-8 overflow-hidden">
            ...
            {data && data.drivers.length > 0 && (
                ...
                >
                    <span>
                        Showing <span className="font-medium text-foreground">{filteredDrivers.length}</span> of{' '}
                        <span className="font-medium text-foreground">{data.drivers.length}</span> drivers
                    </span>
                    {isSearching && <span className="text-muted-foreground/60">• filtered</span>}
                </motion.div>
            )}
            ...
            <PremiumCreateDriverDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} />

            <PremiumEditDriverDialog
